(ns intermediation.operation
  "OperationActor -- one insurance-intermediation operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Broker-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Insurance Intermediation
  ...
  One graph run = one insurance-intermediation operation (intake ->
  ...
  decision to a human operator (the licensed agent/broker). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:placement/bind`/`:commission/book` ALWAYS reach this node when the
  governor is clean -- see `intermediation.phase`."
...
(defn build
  ...
  `intermediation.store/Store`).
  opts:
    :advisor      -- an `intermediation.brokerllm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Insurance Intermediation Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
