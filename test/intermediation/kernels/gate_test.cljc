(ns intermediation.kernels.gate-test
  ...
  3. governor boundary — the confidence floor boundary and the
     ...
     through the real `intermediation.governor/check` façade."
  (:require [clojure.test :refer [deftest is testing]]
            [intermediation.governor :as governor]
...
(deftest quotes-floor-pinned
  (is (= 2 gate/quotes-min-required)
      "the best-interest shopping duty is 'at least TWO quotes compared' —
       the kernel's deciding constant must not drift from the documented rule"))
...
(deftest actuation-ops-auto-enabled-nowhere
  (testing "op 4 (:placement/bind) and op 5 (:commission/book) are
...
;; ---------------------------------------------------------------
;; Governor boundary through the real façade. op :placement/intake
;; touches neither the store nor the quote/cap/spec checks, so the
;; verdict is decided purely by confidence/actuation — nil store is
;; safe.

(defn- verdict [proposal]
  (governor/check {:op :placement/intake :subject "placement-x"} {} proposal nil))
...
(deftest actuation-still-escalates-and-hard-still-wins
  ...
  (is (true? (:escalate? (verdict {:confidence 0.99 :stake :actuation/book-commission}))))
  (testing "a hard violation dominates actuation escalation"
    (let [v (governor/check {:op :jurisdiction/assess :subject "placement-x"} {}
