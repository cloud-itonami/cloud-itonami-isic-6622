(ns intermediation.portable-cljs-test-runner
  ...
  `intermediation.store-contract-test`, which exercises the langchain.db
  ...
  -m if combined, hence -Sdeps for the extra path):
    ...
    clojure -Sdeps '{:paths [\"src\" \"test\"]}' \\
      ...
      --output-to target/tests.cjs -c intermediation.portable-cljs-test-runner
  ...
  (:require [clojure.test :as t :refer [run-tests]]
            [intermediation.corporate-intel-test]
            [intermediation.facts-test]
            [intermediation.governor-contract-test]
            [intermediation.kernels.gate-test]
            [intermediation.phase-test]
            [intermediation.registry-test]
            [intermediation.store-contract-test]))
...
(defn -main []
  (run-tests 'intermediation.facts-test
             'intermediation.registry-test
             'intermediation.phase-test
             'intermediation.kernels.gate-test
             'intermediation.governor-contract-test
